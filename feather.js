...
(function (AV) {
    AV.build = {
        ...
        feather_baseURL: "http://feather.aviary.com/2.5.4.169/",
        feather_baseURL_SSL: "https://dme0ih8comzn4.cloudfront.net/2.5.4.169/",
...
(function (AV, window, document) {
    AV.AssetManager = function (isPremiumPartner, inAppPurchaseAllowed) {
        ...
        var _getPartnerAssets = function (type, callback) {
            if (!_assets) {
        ...
        var _getRemotePurchasedAssets = function (type, callback) {
            if (!_assets) {
                api.getAssets(type, function (assets) {
                    _assets = assets;
                    for (var i = 0; i < _assets.length; i++) {
                        _fixAssetURL(_assets[i])
                    }
                    api.getPurchasedAssets(type, callback)
                });
                return false
            }
            try {
        ...
        var _getLocalPurchasedAssets = function (type, callback) {
            _assets = [{
                needsPurchase: false,
        ...
        var api = this;
        api.getAssets = function (type, callback) {
        ...
        api.getPurchasedAssets = isPremiumPartner ? _getPartnerAssets : inAppPurchaseAllowed ? _getRemotePurchasedAssets : _getLocalPurchasedAssets;
...
(function (exports, window, document) {
                ...
                if (AV.build["imgtrackServer"] && AV.JSON) {
                    switch (type) {
                        case "close":
                            ...
                            trackFormJSON = AV.JSON.stringify(trackFormData);
                            break;
                        case "firstclick":
                            trackFormData = {
                                toolusage: usage
                    ...
                    trackForm = AV.controlsWidgetInstance.serverMessaging.sendMessage({
                        id: "avpw_track_form",
                        action: AV.build["imgtrackServer"],
                        method: "GET",
                        keyValues: {
                            action: type,
                            sessionid: this.getUUID(),
                            featherversion: AV.build["version"],
                            apikey: AV.launchData.apiKey,
                            toolusagedata: trackFormJSON
                        },
                        callback: function () {
                            AV.util.nextFrame(function () {
                                var $avpw_img_track_target_holder = avpw$("#avpw_track_form_target_holder");
                                if ($avpw_img_track_target_holder && $avpw_img_track_target_holder.length) {
                                    $avpw_img_track_target_holder.empty()
                                }
                                if (trackForm) {
                                    var targetId = trackForm.attr("target");
                                    avpw$("#" + targetId).unbind("load");
                                    trackForm.remove()
                                }
                            })
    ...
    AV.getActiveTools = function (tools, ignoreArray) {
        ...
        for (i = 0; i < activeToolsAll.length; i++) {
            name = activeToolsAll[i];
            name = AV.publicName2PanelMode(name);
    ...
    AV.paintWidgetLauncher_Flash = function (paintImgIdElem, imgURL) {
        ...
        var activeTools;
        var assetManager;
        activeTools = AV.getActiveTools(AV.launchData.tools, ["whiten", "barrel", "bulge", "pinch", "warmth", "frames", "colorsplash"]);
        ...
        function imgLoadComplete() {
            ...
            AV.controlsWidgetInstance.layoutNotify(AV.launchData["openType"], "placeControls_Flash", [AV.util.getApiVersion(AV.launchData) === 2 ? AV.launchData.appendTo : undefined]);
    ...
    AV.paintWidgetLauncher_HTML = function (paintImgIdElem, imgURL) {
        ...
        AV.controlsWidgetInstance.layoutNotify(AV.launchData["openType"], "placeControls", [AV.util.getApiVersion(AV.launchData) === 2 ? AV.launchData.appendTo : undefined]);
        AV.controlsWidgetInstance.layoutNotify(AV.launchData["openType"], "enableControls");
        avpw$("#avpw_controls").fadeIn(delay);
        AV.util.nextFrame(function () {
            if (AV.launchData["openType"] == "mobile") {
        ...
        var loadImageFromData = function (data) {
            ...
            avpw$(imgElementClean).load(function (e) {
                ...
                if (AV.launchData["actionListJSON"]) {
                    AV.paintWidgetInstance.actions.importJSON(AV.launchData["actionListJSON"])
                }
            }).attr("src", data)
        };
    ...
    AV.ControlsWidget.prototype.setupScrollPanels = function () {
        ...
        var pagerOpts = {
            ...
            itemBuilder: function (i) {
                optionName = _this.activeTools[i];
                if (optionName == "overlay") {
                    capOptionName = AV.getLocalizedString("Stickers")
                } else if (optionName == "barrel") {
                    capOptionName = AV.getLocalizedString("Bulge")
                } else if (optionName == "drawing") {
                    capOptionName = AV.getLocalizedString("Draw")
                } else if (optionName == "textwithfont") {
                    capOptionName = AV.getLocalizedString("Text")
                } else if (optionName == "colorsplash") {
                    capOptionName = AV.getLocalizedString("Splash")
                } else {
                    capOptionName = optionName.substr(0, 1).toUpperCase() + optionName.substr(1)
                }
                capOptionName = AV.getLocalizedString(capOptionName);
                return AV.template[AV.launchData.layout].eggIcon({
                    optionName: optionName,
                    capOptionName: capOptionName
                })
            },
        ...
        this.assetManager.getPurchasedAssets("PERMISSION", function (assets) {
            ...
            if (aviaryBrandingShown) {
                pagerOpts.lastPageTemplate = AV.template[AV.launchData.layout].aviaryScrollPanel;
                pagerOpts.lastPageContents = {
                    panelWidth: pageWidth,
                    panelClass: "avpw_scroll_page_complete"
                };
                avpw$("#avpw_powered_branding").html(AV.template[AV.launchData.layout].poweredByFooterLogo);
    ...
    AV.ControlsWidget.prototype.save = function () {
        ...
        var saveImageToFeatherServices = function () {
            var _this = this;
...
(function (exports, window, document) {
    ...
    AV.defaultTools = ["enhance", "effects", "frames", "overlay", "orientation", "resize", "crop", "warmth", "brightness", "contrast", "saturation", "sharpness", "colorsplash", "drawing", "text", "redeye", "whiten", "blemish"];
    (function (w) {
        var gatherConfigs = function (w, o) {
            o = o || {};
            return {
                apiVersion: 1,
                theme: o["theme"] || w["Feather_Theme"] || "bluesky",
                minimumStyling: o["minimumStyling"] || false,
                openType: "aviary",
                layout: o["layout"] || "desktop_bluesky",
                language: o["language"] || w["Feather_Language"] || "en",
                forceFlash: o["forceFlash"] || w["Feather_ForceFlash"],
                forceSupport: o["forceSupport"] || w["AV_Feather_ForceSupport"],
                poweredByURL: o["poweredByURL"] || "http://www.aviary.com",
                giveFeedbackURL: o["giveFeedbackURL"] || "http://www.aviary.com/feature-requests",
                getWidgetURL: o["getWidgetURL"] || "http://www.aviary.com",
                onLoad: o["onLoad"] || w["Feather_OnLoad"],
                onReady: w["Feather_OnLaunchComplete"],
                onClose: w["Feather_OnClose"],
                onSave: w["Feather_OnSave"],
                onSaveButtonClicked: w["Feather_OnSaveButtonClicked"],
                onError: w["Feather_OnError"],
                image: null,
                url: null,
                appendTo: null,
                noCloseButton: w["Feather_NoCloseButton"],
                maxSize: w["Feather_MaxSize"] || w["Feather_MaxDisplaySize"] || 800,
                maxEditSize: null,
                hiresMaxSize: 1e4,
                hiresWidth: null,
                hiresHeight: null,
                tools: w["Feather_EditOptions"] && w["Feather_EditOptions"] !== "all" && w["Feather_EditOptions"] !== "All" && w["Feather_EditOptions"] !== "" ? w["Feather_EditOptions"] : AV.defaultTools,
                initTool: "",
                cropPresets: w["Feather_CropSizes"] || ["Original", "Custom", ["Square", "1:1"], "3:2", "3:5", "4:3", "4:5", "4:6", "5:7", "8:10", "16:9"],
                cropPresetDefault: "Custom",
                cropPresetsStrict: false,
                resizePresets: w["Feather_ResizeSizes"] || ["320x240", "640x480", "800x600", "1280x1024", "1600x1200", "240x320", "480x640", "600x800", "1024x1280", "1200x1600"],
                stickers: w["Feather_Stickers"] || [],
                ...
                jpgQuality: w["Feather_FileQuality"] || 100,
                debug: false,
                asyncSave: true,
                signature: w["Feather_Signature"] || null,
                timestamp: w["Feather_Timestamp"] || null,
                useCustomFileExpiration: false,
                allowInAppPurchase: false,
                partnerAssetURL: undefined,
                launchDelay: 300,
                closeDelay: 300
            }
        };
        AV.baseConfig = gatherConfigs(w);
        ...
        if (w["Feather_Theme"] || w["Feather_OpenType"] || w["Feather_APIKey"]) {
            w.setTimeout(function () {
                var legacyFeather = new AV.Feather;
                w.aviaryeditor = function (imageID, imageURL, postData, inlineSite) {
                    AV.launchData = gatherConfigs(w, AV.baseConfig);
    ...
    AV.Feather = function (config) {
        ...
        var resetToolsToDefault = false;
        if (config) {
            config.openType = "aviary";
            config.layout = "desktop";
            if (!config.tools || config.tools === "all" || config.tools === "All" || config.tools === "") {
                resetToolsToDefault = true
            }
        }
        config = config || {};
        AV.launchData = AV.util.extend(AV.baseConfig, config);
        (function AV__featherAutoInit() {
            ...
            if (resetToolsToDefault) {
                AV.launchData["tools"] = AV.defaultTools
            }
        ...
        var beginLaunch = function () {
            if (AV.paintWidgetInstance) {
                ...
                var imageEl = AV.util.getImageElem(AV.launchData.image);
                AV.paintWidgetLauncher(imageEl, AV.launchData.url)
            }
        };
        ...
        api.getImageDimensions = function () {
            ...
            if (AV.paintWidgetInstance) {
                ...
                if (!(AV.launchData["hiresUrl"] && AV.launchData["hiresWidth"] && AV.launchData["hiresHeight"])) {
